...
public class TestLab2 {
    ...
    public void testLab2() throws Exception {
        String filePath = System.getProperty("user.dir") + "/resources/软件测试名单.xlsx";
    ...
    private HashMap<User, String> getAccount(String filePath, int startRow, Character accountColumn, Character gitColumn)
        ...
        int accountCol = accountColumn - 'A';  // 获取excel中账号所在列
        int gitCol = gitColumn - 'A';  // 获取excel中git地址所在列
        Workbook wb = new XSSFWorkbook(filePath);
        Sheet sheet = wb.getSheetAt(0);  // 获取存放数据的表
        int maxRow = sheet.getPhysicalNumberOfRows();
        for (int i = startRow - 1; i < maxRow; i++) {
            Row row = sheet.getRow(i);
            if (row != null) {
                NumberFormat NF = NumberFormat.getInstance();
                NF.setGroupingUsed(false);  // 设置读取到的账号的格式
                String account = NF.format(row.getCell(accountCol).getNumericCellValue());
                String password = account.substring(account.length() - 6);  // 根据账号获取密码
